...
class Ingredient
  ...
  def initialize(name, cap, dur, flav, tex, cal)
    @name = name[0..-2]
    @cap = cap.chomp.to_i
    @dur = dur.chomp.to_i
...
def calc(arr, *amounts)
  ...
  return 0 if total_calories != 500
...
File.open("input.txt", "r") { |f| input = f.readlines }

ingredients = []

input.each do |ingredient|
  spl = ingredient.split(" ")
  ingredients << Ingredient.new(spl[0], spl[2], spl[4], spl[6], spl[8], spl[10])
end
